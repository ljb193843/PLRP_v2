...
class Contenedor{	
	...
	string Tipo;
	Ruta *Rut;
	unordered_map<string,double> Combj;
	public:
		//SETTERS
		void set_ID(string ID){
			this->id = ID;
		...
		unordered_map<string,double> get_Combj(){
			return this->Combj;
		};
		bool get_link(){
			return this->Link;
...
void Asignar_Costos(unordered_map<string,map<string,double> > &costos, unordered_map<string,Deposito*> depositos,
					unordered_map<string,Contenedor*> contenedores){
...
/*
map<string,double> BuscarContContMasCercano_True2(map<string,map<string,double> > Costos,map<string,string> &Rutas,string idC1){
...
void AsignarRuta(map<string,map<string,double> > Costos,map<string,Contenedor*> &Cont,map<string,string> &Rutas,
...
double PrintRuta(map<string,Ruta*> Rut,ofstream& data,int tipo_ruta,map<string,map<string,double> >Costos,int &contV){
	...
	for(map_rut=Rut.begin();map_rut!=Rut.end();map_rut++){
		...
		cont =  tipo_ruta*map_rut->second->get_Costos()+ map_rut->second->get_Deposito()->get_costoAp() + cont;
		contV = contV + 1;
...
int main(int argc, char *argv[]){
	...
	unordered_map<string,vector<double> > datosC;
	unordered_map<string,vector<double> > datosD;
	unordered_map<string,double> combu;
	
	unordered_map<string,vector<string> > dias_Cont; 
	
	
	
	
	ifstream  data("Entradas/datosC1.csv");//CONTENEDORES
	ifstream  data2("Entradas/datosD2.csv");//DEPOSITOS
    ...
    cout << "Guardar contenedores" << endl;
    col = 1;
    unordered_map<string,double>::iterator Conte;
    cout << "Pilas" << endl;
    for(Conte=Cont_Pilas.begin()->second->get_Combj().begin();Conte!=Cont_Pilas.begin()->second->get_Combj().end();++Conte){
		cout << (*Conte).first << endl;
		//dias_Cont[Conte->first];
