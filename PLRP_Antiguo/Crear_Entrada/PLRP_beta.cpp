	...
	while(contC!=0){
		...
		if(uso>delimiter){
			if(costoD<costoC){
				...
				contC = contC -1;
				R1->set_Costos(R->get_Costos()+Costos[idC1][idD]);
				Ruta_C[R1->get_Id()]=R1;//Guardar Ruta
			}
			if(costoD>costoC){
				
				//cout << "CostoC1 fue menor" << endl;
				//sleep(1);
				//cout << "Asignar a la ruta" << endl;
				//sleep(1);
				//cout << idC2 << endl;
				//cout << Cont[idC2]->get_Ruta()->get_Id() << endl;
...
int main(int argc, char *argv[]){
	...
	ofstream data4("Salidas/matriz_beta.txt");
