...
class Contenedor{	
	...
	public:
		...
		void set_Combj(map<string,double> combj){
			this->Combj = combj;
		};
...
void Ruta_Greedy(map<string,map<string,double> > Costos,map<string,Deposito*> &depositos,map<string,Contenedor*> &contenedores, 
	...
	//cout << dia << endl;
	//sleep(5);
	//cout << "Contenedor mas cercano y su deposito: " << idC1 << " " << idD << endl;
	//sleep(5);
	...
	while(contC!=0){
		if(contC!=1){
			...
			costoC = BuscarContContMasCercano_True(Costos,Cont,idC1).begin()->second;//Costo del contenedor
			
			//cout << "Cont mas cercano: " << idC1 << endl;
			//sleep(5);
			//cout <<"Cont mas cercano a " << idC1 << ": " << idC2 << endl;
			//sleep(5);
			//cout <<"Dept mas cercano a: " << idC1 << ": " << idD << endl;
			//sleep(5);  
			double uso = (Cont[idC1]->get_capacity(dia)/Cont[idC1]->get_maxcapacity())*100;
			//cout << uso << endl;
			//sleep(5);
		if(uso>delimiter){
			if(costoD<costoC){
				//cout << "CostoD fue menor" << endl;
				//sleep(5);
				//cout << "Crear Nueva Ruta" << endl;
				//CREAR RUTA
				Ruta *R1 = new Ruta;
				R1->set_Id("R"+to_string(contId));//ASIGNAR ID
				contId = contId + 1;//AUMENTAR EL CONTid
				R1->set_Deposito(depositos[idD]);//ASIGNAR DEPOSITO
				//cout << idD << endl;
				R1->set_Vehiculo(depositos[idD]->get_Vehiculo(idD+"_V"+to_string(depositos[idD]->get_contV())));//set Vehiculo
				depositos[idD]->set_contV(depositos[idD]->get_contV()+1);//aumentar contV
				Cont[idC1]->set_Ruta(R1);//asignar ruta
				R1->set_Contenedor(idC1,Cont[idC1]);//set contenedor
				R1->get_Vehiculo()->set_capacity(Cont[idC1]->get_capacity(dia));//set capacidad vehiculo
				Cont[idC1]->set_Link(true);//contenedor ocupado
				contC = contC -1;
				R1->set_Costos(R->get_Costos()+Costos[idC1][idD]);//setear los costos
				Ruta_C[R1->get_Id()]=R1;//Guardar Ruta
			...
			if(costoD>costoC){
				...
				//cout << "CostoC fue menor" << endl;
				//sleep(5);
				//cout << "Asignar a la ruta" << endl;
				//sleep(5);
				//cout << idC2 << endl;
				capV = Cont[idC2]->get_Ruta()->get_Vehiculo()->get_capacity();//capacidad del vehiculo
				maxCapV = Cont[idC2]->get_Ruta()->get_Vehiculo()->get_MaxCapacity();//capacidad maxima del vehiculo
				//cout << "capacidad del vehiculos " << Cont[idC2]->get_Ruta()->get_Vehiculo()->get_Id() << ": " << capV << endl;
				//sleep(5);
				//cout << "capacidad maxima: " << maxCapV << endl;
				//sleep(5); 
				if(capV+Cont[idC1]->get_capacity(dia)<=maxCapV){
					//cout << "cont cabe en vehiculo" << endl;
					//sleep(5);
					//SE ASIGNA A LA RUTA
					Cont[idC2]->get_Ruta()->get_Vehiculo()->set_capacity(capV+Cont[idC1]->get_capacity(dia));//Aumentar cap del vehiculos
					Cont[idC2]->get_Ruta()->set_Contenedor(idC1,Cont[idC1]);//Guardar contenedor en RUTA
					Cont[idC1]->set_Ruta(Cont[idC2]->get_Ruta());//Asignar la ruta al contenedor
					contC = contC -1;
					Cont[idC1]->set_Link(true);//Ocupado
					Cont[idC1]->set_Ruta(Cont[idC2]->get_Ruta());//Asignar la ruta al contenedor
					Cont[idC2]->get_Ruta()->set_Costos(Cont[idC2]->get_Ruta()->get_Costos()+Costos[idC1][idC2]);//Aumentar los costos
				}
				else{
					//cout << "cont no cabe en vehiculos" << endl;
					//sleep(5);
					//cout << "crear nueva ruta" << endl;
					//CREAR RUTA
					Ruta *R1 = new Ruta;
					R1->set_Id("R"+to_string(contId));//SET ID
					contId = contId + 1;//AUMENTAR CONT ID
					R1->set_Deposito(depositos[idD]);//SET DEPOSITO
					R1->set_Vehiculo(depositos[idD]->get_Vehiculo(idD+"_V"+to_string(depositos[idD]->get_contV())));//SET VEHICULO
					depositos[idD]->set_contV(depositos[idD]->get_contV()+1);//AUMENTAR CONTV DEL DEPOT
					R1->set_Contenedor(idC1,Cont[idC1]);//SET CONTENEDOR
					R1->get_Vehiculo()->set_capacity(Cont[idC1]->get_capacity(dia));//SET CAPACIDAD DEL VEHICULO
					Cont[idC1]->set_Ruta(R1);//SET RUTA EN EL CONTENEDOR
					contC = contC -1;
					Cont[idC1]->set_Link(true);//OCUPADO 
					R1->set_Costos(Costos[idC1][idD]);//SET COSTOS
					Ruta_C[R1->get_Id()]=R1;//AGREGAR A LA RUTA C
			...
			//cout << "Queda un solo contenedor" << endl;
			idC2 = BuscarContContMasCercano_True(Costos,Cont,idC1).begin()->first;//Buscar el cont mas cercano a idC1
			...
			if(costoD>costoC){
				...
				if(capV+Cont[idC1]->get_capacity(dia)<=maxCapV){
					//cout << "cont cabe en vehiculo" << endl;
					//sleep(5);
					Cont[idC2]->get_Ruta()->get_Vehiculo()->set_capacity(capV+Cont[idC1]->get_capacity(dia));
					Cont[idC2]->get_Ruta()->set_Contenedor(idC1,Cont[idC1]);
					Cont[idC1]->set_Ruta(Cont[idC2]->get_Ruta());
					contC = contC -1;
					Cont[idC1]->set_Link(true);
					Cont[idC1]->set_Ruta(Cont[idC2]->get_Ruta());
					Cont[idC2]->get_Ruta()->set_Costos(Cont[idC2]->get_Ruta()->get_Costos()+Costos[idC1][idC2]);
				}
				else{
					//cout << "cont no cabe en vehiculos" << endl;
					//sleep(5);
					//cout << "crear nueva ruta" << endl;
					Ruta *R1 = new Ruta;
					R1->set_Id("R"+to_string(contId));
					contId = contId + 1;
					R1->set_Deposito(depositos[idD]);
					R1->set_Vehiculo(depositos[idD]->get_Vehiculo(idD+"_V"+to_string(depositos[idD]->get_contV())));
					depositos[idD]->set_contV(depositos[idD]->get_contV()+1);
					R1->set_Contenedor(idC1,Cont[idC1]);
					R1->get_Vehiculo()->set_capacity(Cont[idC1]->get_capacity(dia));
					Cont[idC1]->set_Ruta(R1);
					contC = contC -1;
					Cont[idC1]->set_Link(true);
					R1->set_Costos(Costos[idC1][idD]);
					Ruta_C[R1->get_Id()]=R1;
...
double PrintRuta(map<string,Ruta*> Rut,ofstream& data,int &contVh,int tipo_ruta){
...
int main(int argc, char *argv[]){
    ...
    while(std::getline(data2,line))
    {
        ...
        while(std::getline(lineStream,cell,',')){
			if(cell!="id_D" && cell!="CX" && cell!="CY" && cell!="V"){
			    switch(col){
				    case 1://ID
						D->set_id("D"+cell);
						col = col + 1;
	...
	Asignar_Costos(Costos,Depositos,Contenedores,data3);
	double cont;
	int start_s=clock();
	...
	ofstream data4("Salidas/matriz_alpha.txt");
	for(int i=0;i<3;i++){
		if(i==0){
			cout << "Rutas Pilas dia " << dias[i] << endl;
			Ruta_Greedy(Costos,Depositos,Contenedores,Cont_Pilas,Ruta_Pilas,dias[i],20);
			cont = PrintRuta(Ruta_Pilas,data4,cont1,2) +  cont;
		...
		if(i==1){
			cout << "Rutas Pilas dia " << dias[i] << endl;
			Ruta_Greedy(Costos,Depositos,Contenedores,Cont_Pilas,Ruta_Pilas,dias[i],20);
			cont = PrintRuta(Ruta_Pilas,data4,cont2,2) + cont;
			Reboot(Cont_Pilas,Depositos);
			Ruta_Pilas.clear();
		}
		if(i==2){
			cout << "Rutas Carton dia " << dias[i] << endl; 
			Ruta_Greedy(Costos,Depositos,Contenedores,Cont_Carton_papeles,Ruta_Carton_papel,dias[i],20);
			cont = PrintRuta(Ruta_Carton_papel,data4,cont3,2) + cont ;
			Reboot(Cont_Carton_papeles,Depositos);
			Ruta_Carton_papel.clear();
		}
	}
	int stop_s=clock();
	...
	data4 << "Tiempo de la Simulacion: " << (stop_s-start_s)/double(CLOCKS_PER_SEC) <<" segundos" << "\n";
 	cout << "Costo Total: " << cont+((cont1+cont2+cont3)*500) << endl;
 	data4 << "Costo Total: "<< cont+((cont1+cont2+cont3)*500) << "\n";
	cout << "Vehiculos Involucrados: " << cont1+cont2+cont3 << endl;
	data4 << "Vehiculos Involucrados: " << cont1+cont2+cont3 << "\n";
